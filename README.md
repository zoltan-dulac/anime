...
![Basic animation](/site/img/gifs/basic-anim.gif)
...
![Specific parameters](/site/img/gifs/specific-parameters.gif)
...
![Multi timings](/site/img/gifs/multi-timings.gif)

Delays and durations can be specific to targeted elements using a function.
...
### Valid animatable properties
Animate pretty much any kind of properties, as long as the property value contains at least one numerical value.
...
#### Single value
Define the end value of the animation.
...
![Random values](/site/img/gifs/random-values.gif)

Property values  can be specific to targeted elements using a function.
...
![Playback controls](/site/img/gifs/playback-controls.gif)
...
![Follow path](/site/img/gifs/follow-path.gif)

Snap the transforms properties to an SVG path.
```javascript
...
```javascript
anime.list; // return an array of all active animations
```

### anime.speed
Change global animations speed (from 0 to 1).
```javascript
...
```javascript
anime.easings; // Return the complete list of anime.js easing functions
```
